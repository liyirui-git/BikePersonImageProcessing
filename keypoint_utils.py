...
def get_heatmaps_matrix(pose_data, debug=False):
    '''
    Note:
        ...
        But what the 56 means?

    '''
    heatmaps_path = pose_data["heatmaps_path"]
    print("heatmaps_path: ", heatmaps_path)
    ...
    print(str((hm_width, hm_height)))
    heatmaps_total_image = cv2.imread(heatmaps_path, cv2.IMREAD_GRAYSCALE)
    heatmap_image = numpy.zeros((hm_height, hm_height),dtype=numpy.uint8)

    for i in range(19):
        # 比如 当i==0的时候，对应的是18和19张图像（从0开始编号）
        i1 = 18 + (i*2)
        i2 = 18 + (i*2 + 1)
        for j in range(hm_height):
            for k in range(hm_height):
                k1 = k + i1*hm_height
                k2 = k + i2*hm_height
    ...
    # cv2.imwrite("test_heatmaps_matrix.png", heatmap_image)
    
    heatmap_color = cv2.applyColorMap(heatmap_image, cv2.COLORMAP_JET)
    # cv2.imwrite("test_heatmaps_matrix_color.png", heatmap_color)
    ...
    # cv2.imwrite("test_heatmaps_matrix_binary.png", heatmap_binary)

    _, heatmap_otsu = cv2.threshold(heatmap_image, 0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    # cv2.imwrite("test_heatmaps_matrix_otsu.png", heatmap_otsu)

    heatmap_image = cv2.cvtColor(heatmap_image, cv2.COLOR_GRAY2RGB)
    heatmap_binary = cv2.cvtColor(heatmap_binary, cv2.COLOR_GRAY2RGB)
    heatmap_otsu = cv2.cvtColor(heatmap_otsu, cv2.COLOR_GRAY2RGB)
    origin_image = cv2.imread(pose_data['image_path'])
    pose_image = cv2.imread(pose_data['rendered_image_path'])
    
    heatmap_total = numpy.hstack((origin_image, pose_image, heatmap_image, heatmap_color, heatmap_binary, heatmap_otsu))
